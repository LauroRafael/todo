...
function timeToHours(time: string): number {
  ...
  return h + Math.round((m / 60) * 100) / 100;
}
...
function StatusBadge({ status }: { status: TaskStatus }) {
  ...
  return <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${colors[status]}`}>{labels[status]}</span>;
}
    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-950">
      <form onSubmit={handleLogin} className="w-full max-w-sm rounded-2xl border border-slate-800 bg-slate-900 p-8">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full rounded-xl bg-indigo-600 py-2 text-sm font-medium text-white hover:bg-indigo-500 disabled:opacity-60"
        >
...
function ApontamentosPanel({ task, onClose, onUpdate }: { task: Task; onClose: () => void; onUpdate: () => void }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        toast("error", e instanceof Error ? e.message : "Erro ao carregar apontamentos");
        console.error("[ApontamentosPanel:load]", e);
      } finally {
        setLoading(false);
      }
    }
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setSaving(true);
    try {
      const hoursSpent = timeToHours(draft.workTime);
      ...
      setDraft({ content: "", workDate: new Date().toISOString().split("T")[0], workTime: "" });
      onUpdate();
      toast("success", "Apontamento adicionado!");
    } catch (e) {
      toast("error", e instanceof Error ? e.message : "Erro ao criar apontamento");
      console.error("[ApontamentosPanel]", e);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function onDelete(a: Apontamento) {
    ...
    try {
      ...
      console.error("[ApontamentosPanel:delete]", e);
    }
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col rounded-2xl border border-slate-700 bg-slate-900">
        ...
        <form className="shrink-0 border-b border-slate-700 px-6 py-4" onSubmit={onSubmit}>
          <div className="grid gap-3 rounded-xl border border-slate-700 bg-slate-800/50 p-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <DatePicker
                  ...
                  onChange={(date: Date | null) =>
                    setDraft((d) => ({ ...d, workDate: date ? date.toISOString().split("T")[0] : "" }))
                  }
              ...
              <div>
                <label className="mb-1 block text-xs text-slate-400">Tempo</label>
                <input
                  type="time"
                  step="60"
                ...
                <li key={a.id} className="rounded-lg border border-slate-700 bg-slate-800/50 p-3">
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="rounded bg-indigo-900/40 px-2 py-0.5 text-xs text-indigo-200">
                        {formatHours(a.hoursSpent)}
                      </span>
                      <button onClick={() => onDelete(a)} className="text-xs text-red-400 hover:text-red-300">
                        Excluir
                      </button>
                    </div>
                  </div>
...
export function App() {
  ...
  async function onToggleCompleted(task: Task) {
    ...
    try {
      const updated = await api.updateTask(task.id, {
        ...
        status: newCompleted ? "concluída" : task.status === "concluída" ? "em_execução" : undefined
      });
  ...
  async function onChangeStatus(task: Task, status: TaskStatus) {
    try {
      const patch: Parameters<typeof api.updateTask>[1] = { status };
      // Sincroniza o checkbox: ao concluir via select, marca como concluída;
      // ao sair de "concluída", desmarca automaticamente.
      if (status === "concluída" && !task.completed) patch.completed = true;
      else if (status !== "concluída" && task.completed) patch.completed = false;
      ...
      <div className="mx-auto max-w-4xl px-4 py-10">
        <header className="mb-8 flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span
                className={["h-2 w-2 rounded-full", health?.ok ? "bg-emerald-400" : "bg-slate-500"].join(" ")}
                aria-hidden="true"
              />
            ...
            <button
              ...
              className="rounded-lg border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-200 hover:bg-slate-800 disabled:opacity-60"
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {editingTask ? (
                <button
                  type="button"
                  onClick={cancelEdit}
                  className="rounded-xl border border-slate-800 bg-slate-950/40 px-4 py-2 text-sm text-slate-200 hover:bg-slate-900"
                >
                  Cancelar
                </button>
              ) : null}
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900/40">
          ...
          {!loading && tasks.length === 0 ? (
            <div className="p-4 text-sm text-slate-400">Nenhuma task ainda.</div>
          ) : null}
            ...
            {tasks.map((t) => (
              <li key={t.id} className="px-4 py-4">
                <div className="flex items-start gap-3">
                  <button
                    onClick={() => void onToggleCompleted(t)}
                    className={[
                      "mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded border",
                      t.completed ? "border-emerald-500 bg-emerald-500/20" : "border-slate-700 bg-slate-950/40"
                    ].join(" ")}
                    aria-label={t.completed ? "Marcar como não concluída" : "Marcar como concluída"}
                  >
                    {t.completed ? (
                      <svg
                        viewBox="0 0 20 20"
                      ...
                      >
                        <path
                          fillRule="evenodd"
                          d="M16.704 5.29a1 1 0 010 1.416l-7.25 7.25a1 1 0 01-1.414 0l-3.25-3.25a1 1 0 111.414-1.414l2.543 2.543 6.543-6.543a1 1 0 011.414 0z"
                          clipRule="evenodd"
                        />
                      </svg>
                    ) : null}
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        <p
                          className={["text-sm", t.completed ? "text-slate-400 line-through" : "text-slate-100"].join(
                            " "
                          )}
                        >
                          {t.title}
                    ...
                    {t.description ? (
                      <p className="mb-3 whitespace-pre-wrap text-xs text-slate-400">{t.description}</p>
                    ) : null}
                    ...
                    <div className="mb-3 flex flex-wrap items-center gap-4 rounded-lg border border-slate-800 bg-slate-950/30 p-3">
                      ...
                      <div>
                        ...
                        <span
                          className={
                            "text-sm " +
                            (t.estimatedHours > 0 && t.executedHours > t.estimatedHours
                              ? "text-red-400"
                              : "text-slate-300")
                          }
                        >
                          {t.estimatedHours > 0 && t.executedHours > t.estimatedHours
                            ? `+${(((t.executedHours - t.estimatedHours) / t.estimatedHours) * 100).toFixed(2)}%`
                            : "0%"}
                        </span>
                      </div>
                      <div className="flex-1">
                        ...
                        <div className="mt-1 flex items-center gap-2">
                          <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-800">
                            <div
                              className={
                                "h-full transition-all " + (calculatePercent(t) > 100 ? "bg-red-500" : "bg-indigo-500")
                              }
                              style={{ width: Math.min(100, calculatePercent(t)) + "%" }}
                            />
                          </div>
                          <span
                            className={"text-sm " + (calculatePercent(t) > 100 ? "text-red-400" : "text-slate-300")}
                          >
                            {calculatePercent(t)}%
                          </span>
                        </div>
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <select
                        ...
                        className="rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-300 focus:outline-none focus:border-slate-500"
                      >
                        {taskStatuses.map((s) => (
